...
class Widget2():
    def __init__(self, scr,s,x,y,r1,r2,v1,v2): #尺寸，x轴偏移量，y轴偏移量，动画起始值，动画结束值，开始处蓝色区域最大值，结尾处红色区域最小值
        ...
        # 创建一个标签label
        self.label = lv.label(scr)
        self.label.set_text("ADC_32")  # 默认值
        self.label.align_to(self.meter, lv.ALIGN.OUT_BOTTOM_MID, 0, 5)  # label的中间与滑块的上外边框中间对齐，然后y向下5像素 x不变
...
class Widget3():
    def __init__(self, scr,s,x,y,r1,r2,v1,v2): #尺寸，x轴偏移量，y轴偏移量，动画起始值，动画结束值，开始处蓝色区域最大值，结尾处红色区域最小值
        ...
        # 创建一个标签label
        self.label = lv.label(scr)
        self.label.set_text("ADC_35")  # 默认值
        self.label.align_to(self.meter, lv.ALIGN.OUT_BOTTOM_MID, 0, 5)  # label的中间与滑块的上外边框中间对齐，然后y向下5像素 x不变
...
class Widget4():
    def __init__(self, scr,s,x,y,r1,r2,v1,v2): #尺寸，x轴偏移量，y轴偏移量，动画起始值，动画结束值，开始处蓝色区域最大值，结尾处红色区域最小值
        ...
        # 创建一个标签label
        self.label = lv.label(scr)
        self.label.set_text("ADC_34")  # 默认值
        self.label.align_to(self.meter, lv.ALIGN.OUT_BOTTOM_MID, 0, 5)  # label的中间与滑块的上外边框中间对齐，然后y向下5像素 x不变
    ...
    def timer_cb(self, timer=None):
        """定时器回调函数"""
        ret=round(100*adc4.read()/4095)
        self.meter.set_indicator_value(self.indic, ret)
        return ret    
       
        
# 3. 创建要显示的组件
Widget1=Widget1(scr,105,-55,-90,0,100,0,90)#尺寸，x轴偏移量，y轴偏移量，动画起始值，动画结束值，开始处蓝色区域最大值，结尾处红色区域最小值
Widget2=Widget2(scr,105,55,-90,0,100,0,90)
Widget3=Widget3(scr,105,-55,50,0,100,0,90)
Widget4=Widget4(scr,105,55,50,0,100,0,90)
