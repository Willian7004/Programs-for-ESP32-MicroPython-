...
# 2. 封装要显示的组件
class MyWidget():
    def __init__(self, scr,x,y,w,h,rmin,rmax,t,value):  #x轴偏移量，y轴偏移量，宽度，高度，上升时间，数值
        ...
        self.bar.set_range(rmin, rmax)

        # 4. 创建动画对象
        anim_obj = lv.anim_t()
        anim_obj.init()
        anim_obj.set_var(self.bar)
        anim_obj.set_values(rmin, value)
        anim_obj.set_time(t)  # 设置从当前效果到指定效果的过度时间
        #anim_obj.set_playback_time(200)  # 设置从指定效果到之前效果的过度时间
        # anim_obj.set_repeat_count(lv.ANIM_REPEAT_INFINITE)  # 设置重复
        anim_obj.set_custom_exec_cb(self.set_bar)  # 设置动画回调函数
        ...
        self.label.set_text(str(value))  # 默认值
        self.label.align_to(self.bar, lv.ALIGN.OUT_BOTTOM_MID, 0, 5)  # label的中间与滑块的上外边框中间对齐，然后y向下5像素 x不变
    
    def set_bar(self, anim_obj, value):
        self.bar.set_value(value, lv.ANIM.ON)
        
# 3. 创建要显示的组件
Widget1=MyWidget(scr,-50,0,20,200,0,50,200,15)  #x轴偏移量，y轴偏移量，宽度，高度，上升时间，数值
Widget2=MyWidget(scr,0,0,20,200,0,50,200,25)
Widget3=MyWidget(scr,50,0,20,200,0,50,200,35)
