...
class MyWidget():
    def __init__(self, scr,s,x,y,r1,r2,v1,v2,t1,t2,t3,t4): #尺寸，x轴偏移量，y轴偏移量，动画起始值，动画结束值，开始处蓝色区域最大值，结尾处红色区域最小值，上升时间，延时，下降时间，延时
        ...
        # 7:短线长度
        # 最后1个参数：颜色
        self.meter.set_scale_ticks(scale, 51, 1, 7, lv.palette_main(lv.PALETTE.GREY))
        # -------- 主刻度线 --------
        # 10: 多少个子刻度线显示1个主刻度线
        # 2: 宽度
        # 10: 长度
        # 下一个参数：颜色
        # 10: 文字与线的距离 10像素
        self.meter.set_scale_major_ticks(scale, 10, 2, 10, lv.color_black(), 10)
...
# 3. 创建要显示的组件
Widget1=MyWidget(scr,140,0,-75,0,100,10,90,1000,50,1000,50)#尺寸，x轴偏移量，y轴偏移量，动画起始值，动画结束值，开始处蓝色区域最大值，结尾处红色区域最小值，上升时间，延时，下降时间，延时
Widget2=MyWidget(scr,140,0,75,0,100,20,80,500,25,500,25)
